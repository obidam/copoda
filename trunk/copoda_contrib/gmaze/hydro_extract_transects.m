...
switch cruise
	...
	case '74AB62_1' % AR07E (1991)	
		...
		ii1 = select_1transect(x,y,dl-30,56); % to be removed
		ii2 = select_1transect(x,y,dl-20,53,200,90); % to be removed
		
		ij     = find(x<=dl-29.9 & x>=dl-30.1);
		[a ia] = min(y(ij));
		ii3     = find(x<=dl-29.9 & x>=dl-30.1 & y~=y(ij(ia)));
		
		ij     = find(x<=dl-19.9 & x>=dl-20.1);
		[a ia] = min(y(ij));
		ii4     = find(x<=dl-19.9 & x>=dl-20.1 & y~=y(ij(ia)));
	...
	case 'Meteor 395' % AR07E (1997)
		x0 = dl-42.5;
		y0 = 60;
		ii1 = select_1transect(x,y,dl-42.5,60); % to keep		
		ii2 = select_1transect(x,y,dl-30,51); % to be removed
		ii3 = select_1transect(x,y,dl-22,51); % to be removed
		nokeep = zeros(1,length(x));
		for ipt = 1 : length(x)
